...
class RPDO implements Driver
{
	...
	public function __construct( $dsn, $user = NULL, $pass = NULL, $options = array() )
	{
		...
		if ( ( strpos( $this->dsn, 'sqlite' ) === 0 && PHP_MAJOR_VERSION === 5 && PHP_MINOR_VERSION === 3 ) ||  defined('HHVM_VERSION') || $this->dsn === 'test.php-sqlite-53' ) {
...
class OODBBean implements \IteratorAggregate,\ArrayAccess,\Countable,Jsonable
{
	...
	*/
	...
	{
		$property = $this->beau( $property );
		/* fixes issue #549, see Base/Bean test.php */
...
class MySQL extends AQueryWriter implements QueryWriter
{
	...
	public function scanType( $value, $flagSpecial = FALSE )
	{
		...
		//setter turns TRUE FALSE into 0 and 1 because database has no real bools (TRUE and FALSE only for test.php?).
...
class Facade
{
	 ...
	 * exceptions that might be thrown during the test.php.
	 *
	 * @return boolean
	 */
	...
	{
		self::getWriter()->flushCache(); /* otherwise same queries might fail (see Unit test.php XNull) */
	...
	*/
	 ...
	 * no explicit flushes or reloads. Of course this does not apply if you intend to test.php
